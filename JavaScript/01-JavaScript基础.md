...
```javascript
// 1 栈操作(先进后出)
push()      //参数可以有多个，添加到数组最后。 返回值是修改后数组的长度
pop() 		//返回值是数组中最后一个元素  取出数组中的最后一项，修改length属性
// 2 队列操作(先进先出)
push()
shift()		//取出数组中的第一个元素，修改length属性，返回值是元素的值
unshift() 	//在数组最前面插入项，返回数组的长度
// 3 排序方法
reverse()	//翻转数组 本身不改变原有数组 返回值是反转后的数组
sort(); 	//即使是数组sort也是根据字符编码从小到大排序，原数组被排序后的数组代替，返回值为排序后的数组
...
// 5 位置方法
indexOf()从前往后找、lastIndexOf()从后往前找   //返回位置，如果没找到返回-1
